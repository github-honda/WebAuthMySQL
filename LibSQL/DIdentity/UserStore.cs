...
namespace LibSQL.DIdentity
{
        ...
        where TUser : User
    {
        ...
        public UserStore(string sConnectionString)
        {
            //string sConnectionString = CLibSQL.GetConnectionString();
            new UserStore<TUser>(new ZMySqlClient(sConnectionString));
        }

        public UserStore(ZMySqlClient db1)
        {
            mDB = db1;
            userTable = new UserTable<TUser>(db1);
            roleTable = new RoleTable(db1);
            userRolesTable = new UserRolesTable(db1);
            userClaimsTable = new UserClaimsTable(db1);
            userLoginsTable = new UserLoginsTable(db1);
        }
